        ...
        if key==13: #enter key
            ...
            if os.path.exists(name):
                ...
                for k in reader:
                    ...
                    cor.append([oldone,newone])
                    result=cor+oldbox #렉이 한번 있는데 cor문제는 아니고 기존이미지 저장하는 작동원리때문임
                for r in result:
                    disp=drawBox(reimg,r)
                    paramold.append(disp)
                    reimg=disp
                cv2.imshow('frame',disp) 
                cv2.setMouseCallback('frame',mouse_callback,[reimg]) #img 다시 인수로 넣어주기 
